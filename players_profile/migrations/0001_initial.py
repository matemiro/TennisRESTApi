# Generated by Django 3.2 on 2022-03-03 22:58

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Profile',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(blank=True, default='', max_length=20)),
                ('surname', models.CharField(blank=True, default='', max_length=30)),
                ('age', models.PositiveSmallIntegerField(null=True)),
                ('gender', models.CharField(blank=True, choices=[('FEMALE', 'female'), ('MALE', 'male')], max_length=20)),
                ('racquet', models.CharField(blank=True, default='', max_length=20)),
                ('city', models.CharField(blank=True, default='', max_length=30)),
                ('favorite_surface', models.CharField(blank=True, choices=[('CLAY', 'clay'), ('HARD', 'hard'), ('GRASS', 'grass')], max_length=20)),
                ('user', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
        ),
    ]
